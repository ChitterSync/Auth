...
export async function POST(req: NextRequest) {
  try {
    ...
    const { username, settings } = (await req.json().catch(() => ({}))) as UserSettingsPayload;
    if (typeof username !== 'string' || !username || typeof settings !== 'object' || settings === null) {
    ...
    const saved = await prisma.userSetting.upsert({
      ...
      create: {
        userId: user.id,
        preferences: serializePreferences(normalizedSettings),
      },
      update: {
        preferences: serializePreferences(normalizedSettings),
      },
